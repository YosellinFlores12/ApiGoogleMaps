...
class DatabaseHelper {
  ...
  final String columnaEsAdmin = 'esAdmin'; // Nuevo campo
  ...
  Future<void> _crearTabla(Database db, int version) async {
    await db.execute('''
      CREATE TABLE $_tablaUsuarios (
        ...
        $columnaEsAdmin INTEGER NOT NULL DEFAULT 0
      )
    ''');
  ...
  Future<void> _insertarUsuarioAdmin(Database db) async {
    Map<String, dynamic> usuarioAdmin = {
      columnaNombre: 'Yosellin',
      columnaApellido: 'Martinez',
      columnaCorreo: 'admin@gmail.com',
      columnaContrasena: 'abcd1234',
      columnaEsAdmin: 1,
    };

    await db.insert(_tablaUsuarios, usuarioAdmin);
  }
